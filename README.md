<!--
**shravanshaha19/shravanshaha19** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
# Hi, I'm Shravan Shaha 👋

🎓 BBA (Computer Application) Graduate  
📊 Aspiring Data Analyst / Business Analyst  
☁️ AWS Cloud & Data Analytics Enthusiast  
📍 Maharashtra, India

---

## 🚀 About Me

BBA (Computer Application) graduate with hands-on experience in Data Analytics, Power BI dashboard development, SQL, and Python. Published research author with AWS and Data Analytics certifications, complemented by practical internship experience in data analytics and cloud computing. Skilled in data visualization, data analysis, and problem-solving, with a strong ability to transform data into actionable business insights. Seeking a Data Analyst or Business Analyst role to contribute to data-driven decision-making and organizational growth.

---

## 🛠️ Technical Skills

- Power BI (Dashboards, KPIs, DAX)
- SQL & MySQL
- Python (Pandas, NumPy, Matplotlib)
- Excel (Pivot Tables, VLOOKUP, Power Query)
- AWS Cloud (EC2, S3, VPC)
- Data Cleaning & EDA

---

## 📂 Featured Projects

### 🌍 Smart Environmental Comfort Monitoring System
🔹 Technologies: NodeMCU, IoT Sensors, Embedded Systems  
🔹 IoT-based environmental monitoring system for temperature & humidity tracking  
🔹 Focused on smart automation and real-time monitoring

### 🏋️ ERA Fitness Club – Gym Management System
🔹 Technologies: PHP, MySQL, HTML5, CSS, JavaScript, XAMPP  
🔹 Developed responsive Gym Management System with authentication and membership handling  
🔹 Included trainer management, package registration, and feedback system

### 📊 Bank Loan Analysis Dashboard
- Built interactive Power BI dashboard
- Analyzed loan applications & repayment trends
- Used SQL Server, Excel, and DAX

### 🎬 Netflix Content Data Analysis
- Cleaned 5000+ records using Python
- Performed EDA on genres & release trends
- Created visualizations using Matplotlib

---

## 📄 Research Publication

### [Emerging Trends in Cloud Computing and Cyber Security](https://zenodo.org/records/19510647)

Published in *Journal of Research and Development (JRD), Feb 2026*

* Research Area: Cloud Computing & Cyber Security
* Topics Covered: Multi-Cloud, Zero Trust Security, AI-Based Threat Detection
* Publication Repository: Zenodo

---

## 🏆 Achievements

- 🥈 2nd Rank – Android UI Creation Competition
- 📄 Published Research Paper
- ☁️ AWS Certified
- 📊 Data Analytics Certified

---

## 📫 Connect With Me

- LinkedIn: linkedin.com/in/shravan-shaha-6636452b9
- Email: shravanshaha19@gmail.com
